		...
		success : function(msg) {
			if (msg.error == 1) {
				...
				if ($('#showcart').length > 0) {
					$('#showcart').html(
							'购物车共计' + msg.info.num + '件商品，合计 ' + msg.info.price
									+ '元');
				}
			...
			.ajax({
				...
				success : function(msg) {
					...
					if (msg.error == 1) {// 成功
						...
						cartDiv += '<div><a onclick="closeCart(this);" class="gButton1" href="javascript:void(0);">继续购物 </a><a href="'
								+ cartHostUrl
								+ '/cart/index/" class="gButton2">去结算</a></div>';
						cartDiv += '</div></div>';
						if (i == 1 || i == 0) {
							$(cartDiv).appendTo("#cx_" + This.attr('eid'));
						} else {
							$(cartDiv).appendTo(
									$("#cx_" + This.attr('eid')).parent());
						}
						getCartList();
						car_ie6hack();
...
function yibo(type, product_id, product_num) {
	...
	$.ajax({
		...
		success : function(msg) {
			...
			_adwq.push([ '_setCustomer', json.userid // 1234567是一个例子，请换成当前登陆用户ID或用户名
			]);
			...
			if (json.info) {
				$.each(json.info, function(i, val) {
					if (val) {
						var b = eval('(' + val + ')');
						_adwq.push([ '_setItem', b.product_sn, // 09890是一个例子，请填入商品编号
																// - 必填项
...
function webtrekkSend(webtrekk) {
	...
	wt.sendinfo({
		...
		productCategory : {
			...
			3 : webtrekk.productCategory3.join(";")
		// 请填写商品三级类别名称
		},
