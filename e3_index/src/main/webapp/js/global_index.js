...
(function (d) {
	SFbest.Slide = new function () {
		...
		function Q() {		
			...
			/*
			 * e(); function e() { var p = p || {}; p.u_hover = function (r) {
			 * var q = $(r); q.hover(function () {
			 * $(this).removeClass("hovers").siblings().addClass("hovers") },
			 * function () { $(this).siblings().removeClass("hovers") }) };
			 * p.initFun = function () { p.u_hover("#index_slide .mini_pic a") };
			 * p.initFun() }
			 */
			function m(q) {
				p();
...
(function($) {
	...
	$.alerts = {
		...
		dialogClass: null,                  // if specified, this class will be
	...
	function selectCurrent() {
		...
		if ( options.multiple ) {
			...
			if ( words.length > 1 ) {
				...
				words[wordAt] = v;
				// TODO this should set the cursor to the right position, but it
		...
		$.post("/product/getAllPrice/",{pids:p_ids,eids:e_ids},function(str){
        	if(str){
			    for(var i=0;i<str.length;i++){
					...
					// if($.trim(str[i].tag_img) != '' && (css_id ==
					// 'priceL_')){
					// $("#"+css_id+e_id).parent().append('<span
					// class="icon-cx"><img src="'+str[i].tag_img+'"
					// /></span>');
					// }
...
$(document).ready(function(){
	...
	if ( $(".topshow").length && !getCookie('is_first')){
	  var today = new Date();
	  // setCookie('is_first',1,new Date(today.getTime() + 3 * 60 * 60 *
		// 1000));
	  $(".banner").hide();
	  $(".topclose").hide();
...
/*******************************************************************************
 * describe edm相关js Author zyf Date 2013-09-22
 ******************************************************************************/
$(document).ready(function(){
...
$(".topMenu .menus").dropdown({
	...
	delay: 50},function(){
		/*
		 * var flag = $('#allCategory').attr('flag'); if(0==flag){ $.ajax({
		 * type: 'get', async: false, dataType: 'jsonp', url:
		 * "/ajax/HeadAllCategory/", success: function(data){
		 * $('#allCategory').html(data.data);
		 * $('#floatAllCategory').html(data.data);
		 * $('#allCategory').attr('flag','1'); //alert(data); } }); }
		 */
